...
class Truss:
    ...
    def assembleTrigonometri(self, model: Model, rec: Recorder):
        ...
        for element in structure.list:
            ...
            NODE_1, NODE_2 = (
                element[self.INDEX_NODE_1] - 1,
                element[self.INDEX_NODE_2] - 1,
            )

            length_x = (
                node.list[NODE_2][self.INDEX_POSITION_X]
                - node.list[NODE_1][self.INDEX_POSITION_X]
            )
            length_y = (
                node.list[NODE_2][self.INDEX_POSITION_Y]
                - node.list[NODE_1][self.INDEX_POSITION_Y]
            )
    ...
    def assembleLocalStiffness(self, model: Model, rec: Recorder):
        ...
        for element in structure.list:
            ...
            if rec.pre.localStiffnessMatrix.size == 0:
                ...
                rec.pre.localStiffnessMatrix = np.append(
                    rec.pre.localStiffnessMatrix, [matrix], axis=0
                )
            i = i + 1
    ...
    def assembleGlobalStiffness(self, model: Model, rec: Recorder):
        ...
        for element in structure.list:
            # NODE_1 & NODE_2 are number of nodes for each element
            NODE_1, NODE_2 = (
                element[self.INDEX_POSITION_X],
                element[self.INDEX_POSITION_Y],
            )
            ...
            b1 = 2 * NODE_2 - 2
            b2 = 2 * NODE_2
            rec.pre.globalStiffnessMatrix[a1:a2, a1:a2] += localStiffnessMatrix[
                i, 0:2, 0:2
            ]
            rec.pre.globalStiffnessMatrix[a1:a2, b1:b2] += localStiffnessMatrix[
                i, 0:2, 2:4
            ]
            rec.pre.globalStiffnessMatrix[b1:b2, a1:a2] += localStiffnessMatrix[
                i, 2:4, 0:2
            ]
            rec.pre.globalStiffnessMatrix[b1:b2, b1:b2] += localStiffnessMatrix[
                i, 2:4, 2:4
            ]
            i = i + 1
            pass
    ...
    def assembleUnsolvedMatrix(self, model: Model, rec: Recorder):
        ...
        unsolvedStiffness = rec.pre.globalStiffnessMatrix[
            np.ix_(rec.pre.unrestrainedNode, rec.pre.unrestrainedNode)
        ]
    ...
    def solveDeformation(self, model: Model, rec: Recorder):
        ...
        unknownNodeDeformation = np.linalg.solve(
            rec.pre.unsolvedGlobalStiffnessMatrix,
            rec.pre.unsolvedLoadMatrixWithSettlement,
        )
    ...
    def solveInternalForceStress(self, model: Model, rec: Recorder):
        ...
        for element in structure.list:
            ...
            s = np.dot(
                s,
                rec.post.nodeDeformation[
                    np.ix_(
                        [2 * NODE_1 - 2, 2 * NODE_1 - 1, 2 * NODE_2 - 2, 2 * NODE_2 - 1]
                    )
                ],
            )

            # Force
            f = np.dot(K * A, np.array([[1, -1], [-1, 1]]))
            f = np.dot(f, matrix)
            f = np.dot(
                f,
                rec.post.nodeDeformation[
